                ...
                for i in range(fov_start_index , fov_end_index , fov_increment):
                    ...
                    try:
                        ...
                        if maskindex<11:
                            ...
                            cell_phase = cv2.imread('/Users/Saransh/Documents/2022.03.03/Raffinose/TIFs_Phase/'+str(maskindex)+'.tif',-1)
                            ...
                            #Channel 1 extraction - conversion of channel1 and cell zstack to a horizontal stack that can be analyzed later
                                ...
                                sum_c1 = c1_img #variable to store sum projection for channel 1
                                ...
                                for k in range(start_Index+1,start_Index+int(num_slices)): #add zslices horizontally
                                    ...
                                    cell_img = np.hstack((cell_img,cleaned_bin))
                                    sum_c1 = np.add(sum_c1,curr_img)
                                kernel = np.ones((3,3),np.uint8)
                                cell_img = cv2.dilate(cell_img,kernel,iterations = 1) #to connect mother and daughter cells

                                c1_masked = np.multiply(cell_img,c1_img) #create a cell-masked image of channel 1
                                c1_noised = noisify(c1_masked,mu1,sigma1)
                            ...
                            #Channel 2 extraction - conversion of channel1 and cell zstack to a horizontal stack that can be analyzed late
                                ...
                                for k in range(start_Index+1,start_Index+int(num_slices)): #add zslices horizontally
                                    ...
                                    sum_c2 = np.add(sum_c2,curr_img)

                                c2_masked = np.multiply(cell_img,c2_img) #create a cell-masked image of channel 1
                                c2_noised = noisify(c2_masked,mu2,sigma2)
                                ...
                                if np.shape(cell_size_extractor(cleaned_img))[0]==1: #unbudded cells
                                    ...
                                    f = open('/Users/Saransh/Documents/2022.03.03/2022.05.14_re_analyze/cell_sizes/size_raf_vac.pckl', 'wb')
