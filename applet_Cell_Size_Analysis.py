                    ...
                    try:
                        ...
                        if maskindex<11:
                            ...
                            for j in range(0,l):
                                ...
                                cropped_cell = crop_using_roi_tuple(roi_coordinates,phase_array)
                                ...
                                cropped_cell_img = crop_using_roi_tuple(roi_coordinates,image_array)
                                cropped_cell_img = cropped_cell_img.astype('uint8')

                                cell_img = cleaned_img

                                co_img=np.hstack((cropped_cell,cell_img*1200))
                                ...
                                if np.shape(cell_size_extractor(cleaned_img))[0]==1: #unbudded cells
                                    ...
                                    imwrite(output_path+'/'+channel3+'/'+str(cell_index)+'_cell.tif',co_img)
                                    cell_index+=1
                            ...
                            for j in range(0,l):
                                ...
                                if np.shape(cell_size_extractor(cleaned_img))[0]==1: #unbudded cells
                                    ...
                                    writer.writerow({'major_m':cell_size_extractor(cleaned_img)[0][0]*0.09,'minor_m':cell_size_extractor(cleaned_img)[0][1]*0.09,'state':1}) #cell volume added to the csv file
                                    imwrite(output_path+'/'+channel3+'/'+str(cell_index)+'_cell.tif',co_img,photometric='minisblack')
                                    cell_index+=1
